...
<template>
  <div class="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
    ...
    <div v-for="gap in gaps" :key="gap.id" class="space-y-1">
      <div class="flex justify-between text-xs font-semibold">
        <span class="text-gray-700">{{ gap.description }}</span>
        <span class="text-indigo-600">Relevância: {{ gap.relevance }}/10</span>
      </div>
      <div class="h-1.5 bg-gray-100 rounded-full overflow-hidden">
        <div class="h-full bg-amber-400 rounded-full" :style="{ width: `${gap.relevance * 10}%` }" />
      </div>
    </div>
  </div>
</template>
